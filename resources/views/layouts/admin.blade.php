        ...
        <aside :class="{
            ...
            <div class="h-20 flex items-center border-b border-gray-800 bg-gray-900/50 backdrop-blur-md sticky top-0 z-10 transition-all duration-300"
                ...
                <a href="{{ route('dashboard') }}"
                    class="flex items-center gap-3 transition-transform hover:scale-105 active:scale-95 overflow-hidden">
                    <div
                        class="w-12 h-12 rounded-xl bg-white p-1.5 flex items-center justify-center shadow-lg shadow-indigo-500/10 border border-gray-800 shrink-0">
                    ...
                    <div x-show="!sidebarCollapsed" x-transition:enter="transition ease-out duration-300 delay-100"
                        ...
                        <span
                            class="text-sm font-black text-white tracking-tight leading-none uppercase">{{ App\Models\SystemSetting::get('site_name', 'FeeEasy') }}</span>
                        <span
                            class="text-[8px] font-bold text-indigo-400 uppercase tracking-[0.3em] mt-1">Management</span>
                    </div>
                </a>

                <!-- Desktop Collapse Button (Only visible when expanded) -->
                <button @click="toggleCollapse()" x-show="!sidebarCollapsed"
                    class="hidden lg:block text-gray-500 hover:text-white transition-colors p-1 hover:bg-gray-800 rounded-lg">
                ...
                <button @click="toggleCollapse()" x-show="sidebarCollapsed"
                    class="absolute -right-3 top-7 w-6 h-6 bg-indigo-600 text-white rounded-full flex items-center justify-center shadow-lg hover:bg-indigo-700 transition lg:flex hidden border-2 border-gray-900 z-50">
                ...
                <div class="space-y-1">
                    <a href="{{ route('profile.edit') }}" @click="handleNavigation()"
                        ...
                        @if(request()->routeIs('profile.edit'))
                            <div class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-4 bg-indigo-500 rounded-r-full">
                            </div>
                        @endif
                    </a>
