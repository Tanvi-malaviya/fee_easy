<x-admin-layout title="Institute Details">
    <div class="px-6 py-8">
        <!-- Header Actions -->
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div class="flex items-center gap-4">
                <a href="{{ route('institutes.index') }}" class="p-2 bg-white border border-gray-200 rounded-xl text-gray-500 hover:text-indigo-600 hover:border-indigo-100 transition shadow-sm">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                </a>
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 leading-tight">{{ $institute->institute_name }}</h2>
                    <p class="text-sm text-gray-500 mt-0.5">Institute Identifier: #INST-{{ str_pad($institute->id, 5, '0', STR_PAD_LEFT) }}</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('institutes.edit', $institute) }}" class="inline-flex items-center px-5 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-700 hover:bg-gray-50 transition shadow-sm">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                    Edit Profile
                </a>
                <form action="{{ route('institutes.destroy', $institute) }}" method="POST" onsubmit="return confirm('Archive this institute records?');">
                    @csrf @method('DELETE')
                    <button type="submit" class="inline-flex items-center px-5 py-2.5 bg-red-50 border border-red-100 rounded-xl text-sm font-bold text-red-600 hover:bg-red-100 transition shadow-sm">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden">
                    <div class="px-8 py-6 border-b border-gray-50 flex justify-between items-center bg-gray-50/30">
                        <h3 class="text-xs font-black text-gray-400 uppercase tracking-[0.2em]">Primary Information</h3>
                    ...
                    <div class="p-8">
                        <div class="flex flex-col md:flex-row gap-8">
                            ...
                            <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
                                <div>
                                    <label class="text-[10px] font-black uppercase tracking-widest text-gray-400">Owner / Representative</label>
                                ...
                                <div>
                                    <label class="text-[10px] font-black uppercase tracking-widest text-gray-400">Registration Date</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-10 pt-8 border-t border-gray-50">
                            <div>
                                <label class="text-[10px] font-black uppercase tracking-widest text-gray-400">Full Address</label>
                                <p class="text-base font-semibold text-gray-700 mt-1 leading-relaxed">{{ $institute->address ?? 'N/A' }}</p>
                            </div>
                            <div>
                                <label class="text-[10px] font-black uppercase tracking-widest text-gray-400">City</label>
                                <p class="text-base font-semibold text-gray-700 mt-1">{{ $institute->city ?? 'N/A' }}</p>
                            </div>
                            <div>
                                <label class="text-[10px] font-black uppercase tracking-widest text-gray-400">State & Pincode</label>
                ...
                <div class="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden">
                    <div class="px-8 py-6 border-b border-gray-50 bg-gray-50/30 flex justify-between items-center">
                        <h3 class="text-xs font-black text-gray-400 uppercase tracking-[0.2em]">Latest Subscriptions</h3>
                                ...
                                @forelse($institute->subscriptions as $sub)
                                    <tr class="hover:bg-gray-50/30 transition">
                                        <td class="px-8 py-4">
                                            ...
                                            <div class="text-[10px] text-gray-400">Started: {{ \Carbon\Carbon::parse($sub->start_date)->format('d M, Y') }}</div>
                                        </td>
                                        <td class="px-8 py-4 text-sm font-medium text-gray-700">
                                            {{ \Carbon\Carbon::parse($sub->end_date)->format('d M, Y') }}
                                        </td>
                                        <td class="px-8 py-4 text-sm font-black text-indigo-600">
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden">
                    <div class="px-8 py-6 border-b border-gray-50 bg-gray-50/30">
                        <h3 class="text-xs font-black text-gray-400 uppercase tracking-[0.2em]">Service Eligibility</h3>
                    </div>
                    <div class="p-8">
                        @if($stats['active_subscription'])
                            <div class="p-5 bg-indigo-50 rounded-2xl border border-indigo-100 mb-6">
                                <p class="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Active License</p>
                                <h4 class="text-xl font-black text-indigo-700 mt-1">{{ $stats['active_subscription']->plan_name }}</h4>
                                <p class="text-xs text-indigo-500 mt-2 font-bold uppercase tracking-widest">Expires in {{ \Carbon\Carbon::parse($stats['active_subscription']->end_date)->diffInDays(now()) }} Days</p>
                            </div>
                        @else
