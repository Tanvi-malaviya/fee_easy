    ...
    <div class="max-w-5xl mx-auto py-6">
        ...
        <form action="{{ route('institutes.update', $institute) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-visible">
                <div class="p-4 border-b border-gray-100 bg-gray-50/50 flex items-center gap-4 rounded-t-3xl">
                    <h3 class="text-lg font-bold text-gray-900 ml-4">Institute Profile</h3>
                </div>
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="institute_name" id="institute_name" value="{{ old('institute_name', $institute->institute_name) }}" required 
                            ...
                            class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition px-4 py-2 border text-gray-900 bg-gray-50 focus:bg-white @error('institute_name') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $institute->name) }}" required 
                            oninput="clearError(this)"
                            class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition px-4 py-2 border text-gray-900 bg-gray-50 focus:bg-white @error('name') border-red-500 @enderror">
                        @error('name')
                            <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $institute->email) }}" required 
                            oninput="clearError(this)"
                            class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition px-4 py-2 border text-gray-900 bg-gray-50 focus:bg-white @error('email') border-red-500 @enderror">
                        @error('email')
                            <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="tel" name="phone" id="phone" value="{{ old('phone', $institute->phone) }}" required 
                            oninput="clearError(this)"
                            class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition px-4 py-2 border text-gray-900 bg-gray-50 focus:bg-white @error('phone') border-red-500 @enderror">
                        @error('phone')
                            <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-start space-x-4">
                        <div class="flex-1">
                            ...
                            <input type="file" name="logo" id="logo" accept="image/*" 
                                ...
                                class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 transition">
                            <p class="mt-1 text-xs text-gray-500">Keep empty to retain current logo. Max 2MB.</p>
                        </div>
                        <div id="logo-preview">
                            @if(old('logo_base64'))
                                <div class="mt-6">
                                    <img src="{{ old('logo_base64') }}" class="h-12 w-12 rounded-lg object-cover border border-gray-200 shadow-sm">
                                </div>
                            ...
                            @else
                                <div class="mt-6 w-12 h-12 rounded-lg bg-gray-50 border border-gray-100 flex items-center justify-center">
