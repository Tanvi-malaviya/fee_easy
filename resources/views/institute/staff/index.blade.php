...
@section('content')
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col transition-all scale-95 opacity-0 duration-300"
                ...
                <div class="flex flex-col md:flex-row">
                    ...
                    <form id="salary-form" class="space-y-4">
                        ...
                        <!-- Row 1: Staff Member | Payment Date -->
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <!-- Staff Member -->
                            <div class="space-y-1.5">
                                <label class="block text-[11px] font-bold text-slate-800 uppercase tracking-wider">Select Staff Member</label>
                                <div class="relative" id="salary-staff-dropdown-container">
                                    <button type="button" onclick="toggleSalaryStaffDropdown()" id="salary-staff-selector"
                                        ...
                                        <span id="selected-salary-staff-name" class="text-slate-400">Choose a staff member...</span>
                            ...
                            <!-- Payment Date -->
                            <div class="space-y-1.5">
                                <label class="block text-[11px] font-bold text-slate-800 uppercase tracking-wider">Payment Date</label>
    ...
    <script>
            ...
            const cardsHtml = staffMembers.map(staff => {
                ...
                return `
                    <div class="group bg-white rounded-xl border border-slate-100 hover:shadow-xl hover:shadow-orange-900/5 transition-all duration-300 flex flex-col cursor-pointer relative animate-in fade-in slide-in-from-bottom-2"
                         onclick="if(!event.target.closest('.action-btn')) window.location.href='/institute/staff/${staff.id}'">
                        
                        <div class="pt-4 px-4 pb-2 flex-1 flex flex-col">
                            <!-- Role Badge -->
                            <div class="absolute top-3 right-3">
                                <span class="px-2 py-0.5 bg-slate-50 text-slate-400 text-[9px] font-black rounded-md uppercase tracking-tight">
                                    ${staff.role ? staff.role.name.substring(0, 15) : 'Staff Member'}
                                </span>
                            </div>

                            <!-- Profile Section -->
                            <div class="flex flex-col items-left mb-2 mt-1">
                                <div class="relative w-max mb-2">
                                ...
                                <p class="text-[9px] font-bold text-slate-400 mt-1">${staff.email || 'No Email'}</p>
                            </div>
                        ...
                        <div class="flex items-center justify-between p-3 bg-slate-50/80 rounded-b-xl border-t border-slate-100">
                            <a href="/institute/staff/${staff.id}" class="action-btn flex items-center text-[#006b74] font-bold text-[12px]  transition-all">
                                <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                View
                            </a>
                            <div class="flex items-center gap-3">
                                <button onclick="event.stopPropagation(); openEditModalById(${staff.id})" id="edit-btn-${staff.id}" class="action-btn text-blue-500 hover:text-blue-500 transition-all" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                </button>
                                <button onclick="event.stopPropagation(); deleteStaff(${staff.id}, '${staff.full_name.replace(/'/g, "\\'")}')" class="action-btn text-rose-500 hover:text-rose-500 transition-all" title="Delete">
        ...
        window.selectSalaryStaff = async (id, name, baseSalary) => {
            ...
            if (!document.getElementById('salary_id_input').value) {
                ...
                // Fetch preview for leaves
                try {
                    const monthVal = document.getElementById('salary_payment_date').value;
                    let url = `{{ url('api/v1/institute/salaries/preview') }}/${id}`;
                    if (monthVal) {
                    ...
                    const result = await response.json();
                    if (result.status === 'success') {
                        document.getElementById('salary_leaves').value = result.data.leaves || 0;
                    }
                } catch(e) {
        ...
        window.updateSalaryPreview = () => {
            ...
            const deductions = parseFloat(document.getElementById('salary_leave_deduction_amount').value) || 0;
        ...
        window.saveSalaryRecord = async () => {
            ...
            // Filter out _token and leaves from formData to avoid API validation error
