...
@section('content')
    ...
    <script>
        ...
        function switchStudentTab(tabName) {
            // Hide all tab panes
            document.querySelectorAll('.student-tab-pane').forEach(el => el.classList.add('hidden'));

            // Reset all tab buttons
            document.querySelectorAll('.student-tab-btn').forEach(btn => {
                btn.className = 'student-tab-btn px-4 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 text-slate-600 hover:text-slate-900 hover:bg-slate-50';
            });
            ...
            if (activeBtn) {
                activeBtn.className = 'student-tab-btn px-4 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 bg-primary text-white shadow-xs';
            }
        }
    </script>
