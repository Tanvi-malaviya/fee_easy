...
@section('content')
<div class="space-y-5 max-w-[1600px] mx-auto pb-5">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        <div>
            ...
            <p class="text-sm text-slate-400 mt-2 font-medium">Cohort overview for <span class="text-[#102048] font-bold">{{ $institute->institute_name }}</span>.</p>
        </div>
        <div class="flex items-center gap-4">
            <button onclick="openAddModal()" class="px-6 py-3 bg-[#1e3a8a] text-white rounded-2xl font-bold text-[13px] shadow-lg shadow-blue-900/10 hover:scale-[1.02] transition-transform">
                + Add Student
            </button>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
        <div class="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/20">
            <h3 class="text-sm font-extrabold text-slate-700 uppercase tracking-widest">Student Directory</h3>
    ...
    <div class="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden animate-in fade-in zoom-in duration-300">
        <div class="p-10">
            <div class="flex items-center justify-between mb-10">
                ...
                <button onclick="closeStudentModal()" class="h-10 w-10 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400 hover:text-rose-500 transition-colors">
            ...
            <form id="student-form" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" id="field-name" required placeholder="John Doe" class="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-blue-500/5 transition-all">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[11px] font-extrabold text-slate-400 uppercase tracking-widest ml-1">Email Address</label>
                        <input type="email" name="email" id="field-email" required placeholder="john@example.com" class="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-blue-500/5 transition-all">
                    </div>
                    <div class="space-y-2">
                        <label id="label-password" class="text-[11px] font-extrabold text-slate-400 uppercase tracking-widest ml-1">Password</label>
                        <input type="password" name="password" id="field-password" placeholder="••••••••" class="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-blue-500/5 transition-all">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[11px] font-extrabold text-slate-400 uppercase tracking-widest ml-1">Phone Number</label>
                        <input type="text" name="phone" id="field-phone" placeholder="+123 456 7890" class="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-blue-500/5 transition-all">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[11px] font-extrabold text-slate-400 uppercase tracking-widest ml-1">Assigned Batch</label>
                        <select name="batch_id" id="field-batch" required class="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none appearance-none focus:ring-4 focus:ring-blue-500/5 transition-all">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="standard" id="field-standard" placeholder="e.g. 10th Grade" class="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-blue-500/5 transition-all">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[11px] font-extrabold text-slate-400 uppercase tracking-widest ml-1">Account Status</label>
                        <select name="status" id="field-status" class="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none appearance-none focus:ring-4 focus:ring-blue-500/5 transition-all">
                ...
                <div class="pt-6 border-t border-slate-50 flex items-center justify-end space-x-4">
                    <button type="button" onclick="closeStudentModal()" class="px-8 py-3.5 text-[13px] font-bold text-slate-400">Cancel</button>
...
<div id="delete-modal" class="fixed inset-0 z-[110] flex items-center justify-center hidden">
    ...
    <div class="bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden p-10 text-center animate-in fade-in zoom-in duration-300">
        <div class="h-20 w-20 bg-rose-50 text-rose-500 rounded-full flex items-center justify-center mx-auto mb-6">
        ...
        <p class="text-sm text-slate-400 font-medium mb-8">This action is permanent and will remove all student data.</p>
        <div class="flex items-center space-x-3">
            <button onclick="closeDeleteModal()" class="flex-1 py-3.5 text-[13px] font-bold text-slate-500 bg-slate-50 rounded-2xl">Cancel</button>
            <button id="confirm-delete-btn" class="flex-1 py-3.5 text-[13px] font-bold text-white bg-rose-500 rounded-2xl shadow-lg shadow-rose-200 transition-transform active:scale-95">Delete</button>
        </div>
    </div>
</div>
...
<script>
    ...
    async function fetchStudents(page = 1) {
        toggleLoader(true);
        try {
            // Using direct API V1 endpoint
            const response = await fetch(`/api/v1/institute/students?page=${page}`, {
