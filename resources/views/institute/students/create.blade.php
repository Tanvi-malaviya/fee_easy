    ...
    <div class="max-w-6xl mx-auto">
        <form action="{{ route('institute.students.store') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            ...
                            <div class="space-y-1">
                                <label class="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Date of
                                    Birth</label>
                                <input type="date" name="dob" required value="{{ old('dob') }}" max="{{ date('Y-m-d') }}" onclick="this.showPicker()"
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="text" name="pincode" inputmode="numeric" maxlength="6" pattern="[0-9]{6}" oninput="this.value=this.value.replace(/[^0-9]/g,'').slice(0,6)" value="{{ old('pincode') }}" placeholder="Enter Pincode"
                                class="w-full px-4 py-2.5 bg-slate-50 border border-slate-50 rounded-xl text-sm font-bold outline-none focus:ring-4 focus:ring-blue-500/5 transition-all">
                        </div>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end gap-3 mt-6 mb-10 px-4">
                ...
                <button type="submit"
                    class="px-8 py-2.5 bg-primary text-white rounded-xl font-bold text-sm shadow-lg shadow-orange-900/10 hover:translate-y-[-2px] transition-all">
                    Save Student
                </button>
            </div>
        </form>
