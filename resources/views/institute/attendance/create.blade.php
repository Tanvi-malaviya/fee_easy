...
@section('content')
    <div class="space-y-4 max-w-[1600px] mx-auto pb-6">
        ...
        <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 pb-2">
            <div class="flex items-center">
                <a href="{{ route('institute.attendance.index') }}"
                    class="h-12 w-12 bg-white border border-slate-100 rounded-2xl flex items-center justify-center text-slate-400 hover:text-blue-600 transition-all shadow-sm mr-5 group">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-1">
                    <input type="date" id="attendance-date" value="{{ date('Y-m-d') }}" onchange="loadAttendanceList()"
                        class="px-3 py-1.5 bg-transparent text-[11px] font-bold text-slate-700 outline-none border-none focus:ring-0 cursor-pointer">
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-1">
                    <select id="batch-selector" onchange="loadAttendanceList()"
                        class="px-4 py-1.5 bg-transparent text-[11px] font-bold text-slate-700 outline-none border-none focus:ring-0 min-w-[140px] appearance-none cursor-pointer">
        ...
        <div id="stats-container" class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div class="bg-white px-5 py-4 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
                ...
                <p class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Total Students</p>
                <h3 id="stat-total" class="text-2xl font-black text-slate-800 mt-1">0</h3>
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-tight">Assigned</p>
            </div>
            ...
            <div class="bg-white px-5 py-4 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
                ...
                <p class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Present</p>
                <h3 id="stat-present" class="text-2xl font-black text-emerald-600 mt-1">0</h3>
                <p id="stat-present-label" class="text-[9px] font-bold text-emerald-600/60 uppercase tracking-tight">Rate</p>
            </div>

            <div class="bg-white px-5 py-4 rounded-3xl border border-slate-100 shadow-sm relative overflow-hidden group">
                <div class="absolute top-0 right-0 p-3 opacity-10 group-hover:scale-110 transition-transform">
                    <svg class="w-12 h-12 text-rose-600" fill="currentColor" viewBox="0 0 24 24">
                        <path
                            d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z" />
                    </svg>
                </div>
                <p class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Absent</p>
                <h3 id="stat-absent" class="text-2xl font-black text-rose-600 mt-1">0</h3>
                <p id="stat-absent-label" class="text-[9px] font-bold text-rose-600/60 uppercase tracking-tight">Missing</p>
            </div>
        </div>

        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden relative">
            <!-- Table Toolbar -->
            <div class="px-5 py-3.5 border-b border-slate-50 flex items-center justify-between bg-slate-50/20">
                <div class="flex items-center space-x-4">
                    <h3 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Attendance Registry</h3>
                </div>

                <div id="bulk-actions" class="hidden flex items-center space-x-2">
                    <button onclick="markAllPresent()"
                        class="px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg font-extrabold text-[9px] hover:bg-emerald-100 transition-all flex items-center uppercase tracking-widest">
                        Mark All Present
                    </button>
                    <button onclick="markAllAbsent()"
                        class="px-3 py-1.5 bg-rose-50 text-rose-600 rounded-lg font-extrabold text-[9px] hover:bg-rose-100 transition-all flex items-center uppercase tracking-widest">
                        Mark All Absent
                    </button>
                </div>
            </div>

            <div class="p-4 bg-slate-50/10 border-b border-slate-50 relative">
                <div id="loader-overlay" class="hidden absolute inset-0 bg-white/60 backdrop-blur-[2px] z-50 flex items-center justify-center transition-all duration-300">
                    <div class="flex flex-col items-center">
                        <div class="h-10 w-10 border-4 border-slate-100 border-t-blue-600 rounded-full animate-spin"></div>
                        <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-3">Syncing List...</p>
                    </div>
                </div>
                <div id="attendance-cards-grid" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
                    <div class="col-span-full py-10 text-center text-slate-400 font-medium italic text-[12px]">
            ...
            <!-- Action Bar -->
            <div id="action-bar"
                class="hidden px-8 py-6 bg-slate-50/20 flex items-center justify-end border-t border-slate-50">
                <button id="submit-btn" onclick="submitAttendance()"
                    class="px-12 py-4 bg-[#1e3a8a] text-white rounded-2xl font-bold text-[14px] shadow-xl shadow-blue-900/10 hover:scale-[1.02] transition-transform flex items-center">
    ...
    <script>
        ...
        function renderAttendanceList(students, existingRecords) {
            ...
            container.innerHTML = students.map(student => {
                ...
                return `
                        <div class="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all group animate-in zoom-in-95 duration-300 relative flex flex-col justify-between">
                            <div>
                                <div class="flex items-center mb-4">
                                    <div class="h-8 w-8 rounded-full bg-[#1e3a8a] flex items-center justify-center text-white font-black text-xs tracking-tighter mr-3 shadow-lg shadow-blue-900/20 group-hover:scale-105 transition-transform shrink-0">
                                        ${initials}
                                    </div>
                                    <div class="overflow-hidden">
                                        <h4 class="text-[13px] font-black text-[#1f2937] leading-tight px-1 break-words">${student.name}</h4>
                                        <p class="text-[9px] font-bold text-slate-300 uppercase tracking-widest mt-0.5 px-1">ID: ST-0${student.id}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center space-x-2 mt-auto">
                                <label class="flex-1 cursor-pointer">
                                    ...
                                    <div class="py-2 rounded-lg text-center text-[9px] font-black border border-transparent bg-slate-50 text-slate-400 peer-checked:bg-emerald-50 peer-checked:border-emerald-200 peer-checked:text-emerald-500 transition-all hover:bg-slate-100">PRESENT</div>
                                </label>
                                <label class="flex-1 cursor-pointer">
                                    <input type="radio" name="status_${student.id}" value="absent" 
                                        ...
                                        onchange="calculateStats()" 
                                        class="hidden peer student-status-input" data-id="${student.id}">
                                    <div class="py-2 rounded-lg text-center text-[9px] font-black border border-transparent bg-slate-50 text-slate-400 peer-checked:bg-rose-50 peer-checked:border-rose-100 peer-checked:text-rose-400 transition-all hover:bg-slate-100">ABSENT</div>
                                </label>
                            </div>
                        </div>
                    `;
