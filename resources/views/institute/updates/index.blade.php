...
@section('content')
    <div class="space-y-2 max-w-[1600px] mx-auto pb-6 overflow-x-hidden">
        ...
        <div class="pt-0 px-4 md:px-8 animate-in fade-in duration-500">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-3 md:mb-4">
                <div class="flex-1">
                    <h1 class="text-2xl md:text-4xl font-medium text-slate-800 tracking-tight leading-tight">Updates Hub</h1>
                    <p class="text-xs md:text-sm text-slate-400 mt-1 font-medium leading-relaxed max-w-2xl">Manage and broadcast critical institutional communications, academic calendars, and financial notices to your campus community.</p>
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row items-center gap-4 mb-4 md:mb-4">
                <div class="relative flex-1 w-full group">
                    ...
                    <svg class="w-4 h-4 md:w-5 h-5 absolute left-3.5 md:left-4 top-3 md:top-3.5 text-slate-400 group-focus-within:text-primary transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                    <button type="button" onclick="performSearch()" class="absolute right-1.5 top-1.5 bottom-1.5 px-4 bg-primary text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-slate-700 active:scale-95 transition-all z-20 cursor-pointer">
                        Search
                    </button>
                </div>
                <button onclick="openUpdateModal()" class="w-full md:w-auto px-4 md:px-6 py-2.5 md:py-3 bg-primary text-white rounded-xl font-bold text-sm shadow-lg shadow-orange-900/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2">
                    <svg class="w-4 h-4 md:w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
                    New Update
                </button>
            </div>
    ...
    <div id="update-modal" class="fixed inset-0 z-[100] flex items-center justify-center hidden p-4 sm:p-6">
        ...
        <div class="bg-white w-full max-w-4xl rounded-[1.5rem] shadow-2xl relative z-10 flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300 max-h-[90vh]">
            <!-- Modal Header (Fixed) -->
            <div class="p-4 sm:p-5 border-b border-slate-50 flex items-center justify-between bg-slate-50/20 shrink-0">
                <div>
                    <h2 class="text-lg sm:text-xl font-extrabold text-slate-800 tracking-tight">Post New Update</h2>
                    <p class="text-[10px] sm:text-[11px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">Communication Hub</p>
                </div>
            ...
            <div class="p-4 sm:p-6 overflow-y-auto no-scrollbar">
                <form id="update-form" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-3">
                        <div class="space-y-1">
                            <label class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest ml-1">Recipient</label>
                            <select name="recipient" id="recipient-select" onchange="handleTargetChange()" required class="w-full px-3 py-3 sm:py-2.5 bg-orange-50/50 border border-orange-100 rounded-xl text-xs font-bold text-orange-600 outline-none focus:ring-2 focus:ring-orange-500/20">
                        ...
                        <div id="student-audience-container" class="space-y-1">
                            ...
                            <select name="target_type" id="target-type-select" onchange="handleTargetChange()" required class="w-full px-3 py-3 sm:py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold outline-none">
                        ...
                        <div class="space-y-1">
                            ...
                            <select name="category" required class="w-full px-3 py-3 sm:py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold outline-none">
                    ...
                    <!-- Detail Row -->
                    <div id="audience-detail-row" class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-3">
                        <div id="target-detail-container" class="col-span-1">
                            <div id="batch-selector-container" class="space-y-1 hidden animate-in slide-in-from-top-1">
                                ...
                                <select name="batch_id" id="modal-batch-select" class="w-full px-3 py-3 sm:py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold outline-none">
                            ...
                            <div id="standard-selector-container" class="space-y-1 hidden animate-in slide-in-from-top-1">
                                ...
                                <select name="standard" class="w-full px-3 py-3 sm:py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold outline-none">
                            ...
                            <div id="all-students-placeholder" class="space-y-1 opacity-50">
                                ...
                                <div id="placeholder-text" class="w-full px-3 py-3 sm:py-2.5 bg-slate-100/50 border border-slate-100 rounded-xl text-[10px] font-bold text-slate-400 truncate">Everyone</div>
                            </div>
                        </div>

                        <div class="space-y-1 col-span-1">
                            <label class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest ml-1">Subject</label>
                            <input type="text" name="topic" required placeholder="Main title" class="w-full px-3 py-3 sm:py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold outline-none">
                        </div>

                        <div class="space-y-1 col-span-1">
                            <label class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest ml-1">Attachment (Image/PDF)</label>
                            <input type="file" name="attachment" accept="image/*,application/pdf" class="w-full px-3 py-2 bg-slate-50 border border-dashed border-slate-200 rounded-xl text-[10px] font-bold outline-none file:mr-2 file:py-1 file:px-2 file:rounded-md file:border-0 file:text-[9px] file:font-black file:bg-blue-50 file:text-blue-700">
                        </div>
                    </div>

                    <div class="space-y-1">
                        <label class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest ml-1">Message Content</label>
                        <textarea name="description" required rows="4" placeholder="Write details here..." class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold outline-none resize-none min-h-[100px]"></textarea>
                    </div>

                    <div class="pt-4 flex flex-col-reverse sm:flex-row items-center justify-end gap-3 sm:space-x-3">
                        <button type="button" onclick="closeUpdateModal()" class="w-full sm:w-auto px-6 py-3 sm:py-2.5 text-xs font-bold text-slate-400 hover:text-slate-600">Cancel</button>
                        <button type="submit" id="submit-btn" class="w-full sm:w-auto px-8 py-3 sm:py-2.5 bg-[#ff6c00] hover:bg-[#e05f00] text-white rounded-xl font-bold text-xs shadow-md shadow-orange-500/10 hover:scale-[1.02] active:scale-95 transition-all">
