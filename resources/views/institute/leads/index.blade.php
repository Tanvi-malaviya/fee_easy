...
@section('content')
    <div class="min-h-screen lg:h-[calc(100vh-60px)] bg-[#f3f4f6] md:-mt-6 md:-mx-4 px-4 md:px-8 py-4 md:py-6 overflow-x-hidden lg:overflow-hidden">
        <!-- Header -->
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            <div class="flex items-center gap-4 md:gap-12">
                <h1 class="text-xl md:text-2xl font-black text-gray-800 tracking-tight">Leads Management</h1>
            </div>
        ...
        <!-- Main Content Grid -->
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 h-full lg:h-[calc(100vh-160px)] pb-20 lg:pb-0">

            <!-- Sidebar -->
            <div
                class="col-span-1 lg:col-span-3 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col overflow-hidden h-[400px] lg:h-full">
                <!-- Search Area -->
                ...
                <!-- List Container -->
                <div id="lead-list-container" class="flex-1 overflow-y-auto divide-y divide-gray-100 custom-scrollbar">
                    <!-- Loader -->
                    <div class="p-12 text-center loader-container">
                        <div
                            class="h-8 w-8 border-3 border-gray-100 border-t-[#A8440B] rounded-full animate-spin mx-auto mb-3">
                        </div>
            ...
            <!-- Details Area -->
            <div id="detail-panel"
                class="col-span-1 lg:col-span-9  rounded-xl flex flex-col overflow-hidden min-h-[500px] lg:h-full relative">
                <!-- Empty State -->
                <div id="detail-empty" class="flex-1 flex flex-col items-center justify-center p-8 text-center bg-gray-50/30">
                    <div class="h-20 w-20 bg-gray-100 rounded-full flex items-center justify-center mb-6 text-gray-400">
                        <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122" />
                        </svg>
                    </div>
                    <h3 class="text-base font-bold text-gray-800 mb-2 tracking-tight">Select a Lead</h3>
                    <p class="text-xs text-gray-400 max-w-[240px] leading-relaxed">Choose a lead from the sidebar to view their full profile, interaction history, and analytics.</p>
                </div>

                <!-- Detail Loader -->
                <div id="lead-detail-loader"
                    class="hidden h-full bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col items-center justify-center text-center p-12">
                    <div class="h-12 w-12 border-4 border-gray-100 border-t-[#A8440B] rounded-full animate-spin mb-4"></div>
                    <p class="text-xs font-bold text-gray-400 uppercase tracking-widest">Fetching Details...</p>
                </div>

                <!-- Content Area -->
                <div id="detail-content" class="hidden h-full flex flex-col gap-2 overflow-y-auto pr-4 custom-scrollbar">
                    <!-- Header Card -->
                    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-6">
                                <div id="lead-avatar"
                                    class="h-20 w-20 bg-[#e67e22] rounded-2xl flex items-center justify-center text-2xl font-bold text-white shadow-lg shadow-orange-200">
                                    JM
                                </div>
                                <div>
                            ...
                            <div class="flex items-center gap-3">
                                <button onclick="editLead()"
                                    class="px-5 py-2 bg-white border-2 border-[#007B8A] text-[#007B8A] rounded-xl text-sm font-bold hover:bg-[#007B8A]/5 transition-all flex items-center gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <button onclick="deleteLead()"
                                    class="px-5 py-2 bg-white border-2 border-[#D32F2F] text-[#D32F2F] rounded-xl text-sm font-bold hover:bg-[#D32F2F]/5 transition-all flex items-center gap-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
    ...
    @push('scripts')
        <script>
            ...
            function renderLeadList() {
                console.log('Rendering List, Active ID:', selectedLeadId);
                const container = document.getElementById('lead-list-container');
                if (!container) return;
            ...
            document.getElementById('lead-search')?.addEventListener('keyup', function (e) {
                if (e.key === 'Enter') {
                    fetchLeads();
                }
            });
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (result.data && result.data.id) {
                            selectLead(result.data.id);
                        }
                    } else {
                        const errorData = await response.json();
                        let errorMsg = errorData.message || 'Failed to save lead';
                        if (errorData.errors) {
                ...
                openConfirmModal(
                    ...
                    async () => {
                        try {
                            ...
                            if (response.ok) {
                                selectedLeadId = null;
                                document.getElementById('lead-empty-state').classList.remove('hidden');
            ...
            async function saveNote(event) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        fetchLeads(); // Refresh the list to update last interaction time
                    } else {
                        const err = await response.json();
                        const errDiv = document.getElementById('note-error');
                        errDiv.textContent = err.message || 'Failed to add note';
                        errDiv.classList.remove('hidden');
                    }
                } catch (error) {
