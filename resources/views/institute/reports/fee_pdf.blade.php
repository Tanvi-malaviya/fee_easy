...
<head>
    <meta charset="utf-8">
    <title>Batch Fee Report - {{ $batch->name }}</title>
    <style>
...
<body>
    <div class="header">
        ...
        <p>Batch Fee Report: {{ $batch->name }} ({{ $month }} {{ $year }})</p>
    </div>

    <div class="summary">
        <table>
            <tr>
                <td style="width: 33%;">Total Amount: <br><span class="value">₹{{ number_format($fees->sum('total_amount'), 2) }}</span></td>
                <td style="width: 33%; text-align: center;">Paid Amount: <br><span class="value">₹{{ number_format($fees->sum('paid_amount'), 2) }}</span></td>
                <td style="width: 33%; text-align: right;">Due Amount: <br><span class="value">₹{{ number_format($fees->sum('total_amount') - $fees->sum('paid_amount'), 2) }}</span></td>
            </tr>
        </table>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="font-weight: bold; color: {{ $fee->status === 'Paid' ? '#059669' : ($fee->status === 'Partial' ? '#d97706' : '#dc2626') }}">
                    {{ $fee->status }}
                </td>
            </tr>
            @endforeach
        ...
        <tfoot>
            <tr class="total-row">
                ...
                <td colspan="2">₹{{ number_format($fees->sum('paid_amount'), 2) }}</td>
            </tr>
        </tfoot>
    </table>
