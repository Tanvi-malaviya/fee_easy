...
@section('content')
    ...
    <div class="max-w-7xl mx-auto pb-24">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3 text-[11px] font-bold text-teal-700 uppercase tracking-[0.15em]">
                    <a href="{{ route('institute.batches.homework', $id) }}" class="flex items-center gap-2 hover:text-teal-900 transition-colors">
        ...
        <!-- Progress Overview -->
        <div class="bg-white rounded-3xl p-8 mb-8 border border-slate-100 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                <p class="text-[15px] font-medium text-slate-400"><span id="progress-submitted-count" class="text-orange-700 font-bold">0</span> / <span id="progress-total-count">0</span> Assignments Submitted</p>
            ...
            <div class="flex items-center gap-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                <div class="flex items-center gap-2"><div class="w-2.5 h-2.5 rounded-full bg-orange-700"></div> SUBMITTED (<span id="legend-submitted">0</span>)</div>
                <div class="flex items-center gap-2"><div class="w-2.5 h-2.5 rounded-full bg-teal-200"></div> PENDING (<span id="legend-pending">0</span>)</div>
                <div class="flex items-center gap-2"><div class="w-2.5 h-2.5 rounded-full bg-rose-700"></div> MISSING (<span id="legend-missing">0</span>)</div>
            </div>
        </div>
    ...
    <!-- Floating Action Bar -->
    <div class="fixed bottom-10 right-10 z-[100] flex items-center gap-4 bg-white p-2.5 pl-6 rounded-2xl shadow-2xl shadow-slate-200/50 border border-slate-100 opacity-0 translate-y-4 transition-all duration-300" id="action-bar">
        ...
        <button onclick="publishGrades()" id="publish-btn" class="px-6 py-3 bg-[#c2410c] text-white rounded-xl text-sm font-bold shadow-sm hover:bg-[#a3360a] transition-all">
            Publish Grades
        </button>
    </div>
    ...
    <script>
        ...
        function renderUI() {
            ...
            container.innerHTML = students.map(student => {
                ...
                return `
                    <div class="bg-white rounded-3xl p-6 border ${borderClass} shadow-sm flex flex-col h-full relative">
                        <div class="flex items-start justify-between mb-4">
                            <div class="h-10 w-10 rounded-full bg-slate-800 flex items-center justify-center shrink-0 overflow-hidden">
                        ...
                        <h4 class="text-[15px] font-medium text-slate-900 mb-0.5 truncate">${student.name}</h4>
                        <p class="text-[11px] font-medium text-slate-400 mb-8 truncate">ID: #ST-${student.id.toString().padStart(4, '0')}</p>

                        <div class="mt-auto">
                            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-3">Assignment Score</p>
                            ${isMissing ? `
                                <div class="flex items-center justify-between px-2">
                                    <span class="text-slate-300 font-medium">-</span>
                                    <span class="text-slate-400 font-bold text-[18px] w-12 text-center">0</span>
                                    <span class="text-slate-300 font-medium">+</span>
                                </div>
                            ` : `
                                <div class="bg-slate-50 rounded-xl p-2 flex items-center justify-between border border-slate-100">
                                    <button onclick="updateScore(${student.id}, -1)" class="h-8 w-8 rounded-lg bg-white shadow-sm flex items-center justify-center text-rose-500 font-black hover:bg-slate-100 transition-colors">-</button>
                                    <span id="score-${student.id}" class="text-[18px] font-bold text-slate-900 w-12 text-center">${sub.score}</span>
                                    <button onclick="updateScore(${student.id}, 1)" class="h-8 w-8 rounded-lg bg-white shadow-sm flex items-center justify-center text-orange-600 font-black hover:bg-slate-100 transition-colors">+</button>
                                </div>
                            `}
                        </div>
                    </div>
