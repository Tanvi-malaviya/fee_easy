    ...
    <script>
        ...
        function renderResources() {
            ...
            paginated.forEach(res => {
                ...
                const itemHTML = `
                    <div id="res-${res.id}" onclick="openViewModal(${res.id})" class="bg-white rounded-xl p-3 border border-slate-100 shadow-sm flex flex-col justify-between max-w-[210px] w-full min-h-[220px] relative animate-in fade-in duration-200 cursor-pointer hover:border-primary/40 hover:shadow-md transition-all">
                        <div>
                        ...
                        <div class="flex items-center justify-between mt-3 pt-2 border-t border-slate-50">
                            <a href="javascript:void(0)" onclick="event.stopPropagation(); openViewModal(${res.id})" class="text-[10px] font-bold text-primary hover:underline flex items-center gap-1">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                                View
                            </a>
                            <button onclick="event.stopPropagation(); deleteResource(${res.id})" class="text-slate-400 hover:text-rose-600 transition-colors">
