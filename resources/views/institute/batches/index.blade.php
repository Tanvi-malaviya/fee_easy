    ...
    <div id="list-view" class="space-y-4 animate-in fade-in duration-500">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-1">Manage academic cohorts & enrollment status.</p>
            </div>
            
            <div class="flex items-center gap-2">
                <div class="bg-white px-5 py-3 rounded-[1rem] border border-slate-100 shadow-sm flex flex-col items-center min-w-[120px]">
        ...
        <div class="bg-white p-2 rounded-[1rem] border border-slate-100 shadow-sm flex flex-col md:flex-row items-center gap-2">
            <div class="relative flex-1 group w-full md:w-auto">
                ...
                <svg class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            </div>
            <div class="flex items-center gap-2 w-full md:w-auto">
                <button class="h-10 px-4 bg-white border border-slate-100 text-slate-500 rounded-xl font-bold text-[10px] uppercase hover:bg-slate-50 transition-all flex items-center gap-2">Filter</button>
                <button class="h-10 px-4 bg-white border border-slate-100 text-slate-500 rounded-xl font-bold text-[10px] uppercase hover:bg-slate-50 transition-all flex items-center gap-2">Export</button>
                <button onclick="toggleFormView(true)" class="h-10 px-5 bg-primary2 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest hover:scale-[1.02] transition-transform flex items-center gap-2 whitespace-nowrap">
                    Intilize new batch
                </button>
            </div>
        </div>

        <div id="batch-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 relative min-h-[300px]">
            <div id="loading-spinner" class="absolute inset-0 z-50 bg-white/60 backdrop-blur-[2px] hidden flex items-center justify-center rounded-[1rem]">
                <div class="h-10 w-10 border-4 border-slate-100 border-t-primary rounded-full animate-spin"></div>
            </div>
        </div>

        <div id="pagination-container" class="bg-white p-4 rounded-[1rem] border border-slate-100 shadow-sm flex items-center justify-between"></div>
    </div>
        ...
        <form id="batch-form">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p class="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-1">Configure academic batch details, schedule, and capacity.</p>
                </div>
                <div class="flex items-center gap-3">
                    <button type="button" onclick="toggleFormView(false)" class="h-11 px-6 border-2 border-primary text-primary rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-orange-50 transition-all">Cancel</button>
                    <button type="submit" id="submit-btn" class="h-11 px-6 bg-primary text-white rounded-xl font-bold text-[10px] uppercase tracking-widest shadow-lg shadow-orange-500/20 hover:scale-[1.02] transition-transform flex items-center gap-3">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white p-6 rounded-[1rem] border border-slate-100 shadow-sm space-y-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                <div class="space-y-1.5">
                                    <label class="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">Batch Name</label>
                                    <input type="text" name="name" id="field-name" required placeholder="e.g. Maths 2024" class="w-full px-4 py-3 bg-slate-50/50 border border-slate-100 rounded-xl text-[12px] font-bold outline-none focus:ring-4 focus:ring-primary/5 transition-all">
                                </div>
                                <div class="space-y-1.5">
                                    <label class="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">Subject</label>
                                    <input type="text" name="subject" id="field-subject" required placeholder="Select Subject" class="w-full px-4 py-3 bg-slate-50/50 border border-slate-100 rounded-xl text-[12px] font-bold outline-none focus:ring-4 focus:ring-primary/5 transition-all">
                                </div>
                                <div class="space-y-1.5">
                                    <label class="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">Fees (₹)</label>
                                    <input type="number" name="fees" id="field-fees" placeholder="0" class="w-full px-4 py-3 bg-slate-50/50 border border-slate-100 rounded-xl text-[12px] font-bold outline-none focus:ring-4 focus:ring-primary/5 transition-all">
                                </div>
                            </div>
                            <div class="space-y-1.5">
                                <label class="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">Batch Description</label>
                                <textarea name="description" id="field-description" rows="2" placeholder="Brief details about this cohort..." class="w-full px-4 py-3 bg-slate-50/50 border border-slate-100 rounded-xl text-[12px] font-bold outline-none focus:ring-4 focus:ring-primary/5 transition-all resize-none"></textarea>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4 pt-4 border-t border-slate-50">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-1.5">
                                    <label class="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">Start Time</label>
                                    <input type="time" name="start_time" id="field-start" class="w-full px-4 py-3 bg-slate-50/50 border border-slate-100 rounded-xl text-[12px] font-bold outline-none focus:ring-4 focus:ring-primary/5 transition-all">
                                </div>
                                <div class="space-y-1.5">
                                    <label class="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">End Time</label>
                                    <input type="time" name="end_time" id="field-end" class="w-full px-4 py-3 bg-slate-50/50 border border-slate-100 rounded-xl text-[12px] font-bold outline-none focus:ring-4 focus:ring-primary/5 transition-all">
                                </div>
                            </div>
                            <div class="space-y-3">
                                <label class="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">Days of the week</label>
                                <div class="flex flex-wrap gap-2">
                                    @foreach(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as $day)
                                    <label class="relative cursor-pointer group">
                                        <input type="checkbox" name="days[]" value="{{ substr($day, 0, 3) }}" class="peer sr-only day-checkbox">
                                        <div class="px-4 py-2 bg-white border border-slate-100 rounded-xl text-[11px] font-bold text-slate-400 transition-all peer-checked:bg-secondary/10 peer-checked:text-secondary peer-checked:border-secondary group-hover:bg-slate-50">
                                            {{ $day }}
                                        </div>
                                    </label>
                                    @endforeach
                ...
                <div class="space-y-6">
                    <div class="bg-white p-6 rounded-[1rem] border border-slate-100 shadow-sm space-y-4">
                        <div class="flex items-center gap-2 pb-2 border-b border-slate-50">
                            <div class="h-8 w-8 bg-orange-50 text-primary rounded-lg flex items-center justify-center">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857"/></svg>
                            </div>
                            <h3 class="text-base font-bold text-slate-800 tracking-tight">Capacity</h3>
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">Maximum Students</label>
                            <div class="flex items-center gap-3">
                                <input type="number" name="max_capacity" id="field-capacity" placeholder="30" class="flex-1 px-4 py-3 bg-slate-50/50 border border-slate-100 rounded-xl text-[12px] font-bold outline-none focus:ring-4 focus:ring-primary/5 transition-all">
                                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Seats</span>
                            </div>
                        </div>
                    ...
                    <div class="relative rounded-[1rem] overflow-hidden shadow-lg h-[200px]">
                        <img src="https://images.unsplash.com/photo-1524178232363-1fb2b075b655?q=80&w=2070&auto=format&fit=crop" class="w-full h-full object-cover">
