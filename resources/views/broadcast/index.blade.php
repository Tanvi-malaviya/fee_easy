                ...
                <form action="{{ route('broadcast.index') }}" method="GET" class="flex flex-col md:flex-row gap-4" @submit.prevent="fetchResults()">
                    <div class="flex-1 relative group">
                        <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                            <svg class="h-4 w-4 text-primary group-focus-within:text-primary transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="absolute inset-y-1.5 right-1.5 flex items-center gap-2">
                            <button x-show="search.length > 0" type="button" @click="search = ''; fetchResults()" 
                                class="p-1.5 text-gray-400 hover:text-primary transition" title="Clear">
                                <svg class="w-4 h-4 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </button>
                            <button type="submit" 
                                class="inline-flex items-center px-4 py-1.5 bg-primary text-white rounded-xl font-medium text-[10px] uppercase tracking-widest hover:opacity-90 transition shadow-lg shadow-indigo-600/20 active:scale-95 no-loader">
                                Search
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center">
                        <button type="button" @click="$dispatch('open-modal', 'compose-broadcast')"
                            class="inline-flex items-center px-8 py-3 bg-primary border border-transparent rounded-xl shadow-lg text-xs font-bold text-white uppercase tracking-widest hover:opacity-90 focus:outline-none transition transform active:scale-95 shadow-indigo-600/20 whitespace-nowrap">
            ...
            <div class="relative bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden mb-8">
                ...
                <div x-show="loading" class="absolute inset-0 bg-white/70 backdrop-blur-[2px] z-50 flex items-center justify-center transition-all duration-300" style="display: none;">
                    <div class="flex flex-col items-center gap-3">
                        <div class="relative">
                            <div class="w-12 h-12 rounded-full border-4 border-orange-50"></div>
                            <div class="absolute inset-0 w-12 h-12 rounded-full border-4 border-orange-600 border-t-transparent animate-spin"></div>
                        </div>
                        <span class="text-[10px] font-black text-orange-600 uppercase tracking-[0.2em] animate-pulse">Loading...</span>
                    </div>
                </div>
                    ...
                    <form action="{{ route('broadcast.send') }}" method="POST" enctype="multipart/form-data" class="space-y-3">
                        ...
                        <div>
                            ...
                            <input type="text" id="title" name="title" value="{{ old('title') }}" required
                                class="block w-full px-4 py-2.5 text-sm font-bold text-gray-900 bg-gray-50 border-transparent rounded-xl focus:bg-white focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all duration-300 placeholder:text-gray-300"
                        ...
                        <div>
                            ...
                            <textarea name="message" id="message" rows="3" required
                                class="block w-full px-4 py-2.5 text-sm font-medium text-gray-700 bg-gray-50 border-transparent rounded-xl focus:bg-white focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all duration-300 placeholder:text-gray-300 leading-relaxed"
                        ...
                        <div x-data="{ imagePreview: null }">
                            ...
                            <div class="relative group">
                                ...
                                <label for="image" 
                                    class="flex items-center gap-3 px-4 py-3 bg-gray-50 border-2 border-dashed border-gray-100 rounded-xl cursor-pointer hover:bg-orange-50 hover:border-orange-200 transition-all duration-300 group">
                                    ...
                                    <template x-if="imagePreview">
                                        <div class="relative flex items-center gap-3">
                                            ...
                                            <p class="text-[9px] font-black text-orange-600 uppercase">Photo Selected</p>
                        ...
                        <div x-data="{ target: '{{ old('target', 'all') }}' }">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div @click="target = 'all'"
                                    :class="target === 'all' ? 'border-primary bg-primary/5' : 'border-gray-100 bg-white hover:border-primary/20'"
                                    class="cursor-pointer p-2.5 rounded-xl border-2 transition-all duration-300 group">
                                    <div class="flex items-center gap-2">
                                        <div :class="target === 'all' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-primary/10 group-hover:text-primary'"
                                        ...
                                        <h4 class="text-[10px] font-black text-gray-900 leading-none" :class="target === 'all' ? 'text-primary' : ''">All ({{ $totalInstitutes }})</h4>
                                    </div>
                                </div>
                                <div @click="target = 'subscribed'"
                                    :class="target === 'subscribed' ? 'border-primary bg-primary/5' : 'border-gray-100 bg-white hover:border-primary/20'"
                                    class="cursor-pointer p-2.5 rounded-xl border-2 transition-all duration-300 group">
                                    <div class="flex items-center gap-2">
                                        <div :class="target === 'subscribed' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-primary/10 group-hover:text-primary'"
                                            class="w-6 h-6 rounded-lg flex items-center justify-center transition-colors">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <h4 class="text-[10px] font-black text-gray-900 leading-none" :class="target === 'subscribed' ? 'text-primary' : ''">Subscribed ({{ $subscribedInstitutes }})</h4>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-3 pt-1">
                            ...
                            <button type="submit"
                                class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-white rounded-lg font-black text-[9px] uppercase tracking-widest hover:opacity-90 transition shadow-lg shadow-primary/20 active:scale-95">
